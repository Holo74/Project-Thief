...
namespace Player.Handlers
{
    public partial class CameraShaker : Node3D
    {
        ...
        public void MoveToPositionAndBack(Vector3 pos, float time)
        {
            ...
            Tweening.TweenProperty(PlayerQuickAccess.CAMERA_SHAKE, "position", pos, time / 2).SetEase(Tween.EaseType.Out);
            Tweening.Chain().TweenProperty(PlayerQuickAccess.CAMERA_SHAKE, "position", new Vector3(0, 0, 0), time / 2).SetEase(Tween.EaseType.In);
            Tweening.Play();
        ...
        public void Shake(float x, float y, float z, float step = 0.1f, float time = 0.2f, float amount = 1f, int flip = 1)
        {
            ...
            {
                PropertyTweener t = Tweening.TweenProperty(PlayerQuickAccess.CAMERA_SHAKE, "position", new Vector3(Mathf.Clamp(x - i, 0, x) * flip, Mathf.Clamp(y - i, 0, y) * flip, Mathf.Clamp(z - i, 0, z) * flip), time);
