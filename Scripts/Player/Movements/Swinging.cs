...
namespace Player.Movement
{
    ...
    public partial class Swinging : AbstractMovement
    {
        ...
        {
            return AnchorPoint - Player.PlayerQuickAccess.KINEMATIC_BODY.GlobalPosition;
        }
        ...
        public Swinging(Vector3 anchor, Vector3 right)
        {
            ...
            MaxDistance = Player.PlayerQuickAccess.KINEMATIC_BODY.GlobalPosition.DistanceTo(anchor);
        }
        ...
        private void Swing(double delta)
        {
            ...
            if (!(PlayerQuickAccess.KINEMATIC_BODY.MoveAndCollide(movingTo * ((float)delta) * Speed * (dot - MaxSwingAngle + .1f)) is null))
            {
                Right = -Right;
            }
            PreviousDot = dot;
